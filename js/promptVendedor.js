...
let ingresoArt = confirm(
	"¡Hola vendedor! ¿Desea cargar nuevos artículos a la página?"
);
if (ingresoArt) {
	agregarAlmohadon();
}
...
function agregarAlmohadon() {
	let nombreAlmohadonNuevo = prompt(
		"Ingrese el nombre del almohadón nuevo"
	).toLocaleUpperCase();
	let imagenAlmohadonNuevo = prompt(
		"Ingrese el nombre de la imagen del almohadón"
	).toLocaleUpperCase();
	let colorAlmohadonNuevo = prompt(
		"Ingrese el color del almohadón"
	).toLocaleUpperCase();
	...
	let descripcion = prompt(
		"Ingrese la descripción del almohadón"
	).toLocaleUpperCase();
	...
	let almohadonNuevo = new Almohadon(
		nombreAlmohadonNuevo,
		imagenAlmohadonNuevo,
		colorAlmohadonNuevo,
		medidas,
		precio,
		descripcion,
	...
	//debugger
	let nuevoIngreso = prompt("Si desea agregar un producto nuevo escriba Sí");
	nuevoIngreso == "Si" || "si" || "sí" || "SÍ"
		? agregarAlmohadon()
		: alert(
				"Seleccionaste que no deseas ingresar productos nuevos. Ingresarás a la pagina visualizando los productos cargados anteriormente"
		  );
}
